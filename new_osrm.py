...
def get_osrm_route(coord1, coord2):
    ...
    if data['code'] == 'Ok':
        ...
        print(f"Error retrieving route from OSRM: {data['message']}")
        return []
...
def visualize_route(coordinates, permutation, locations):
    ...
    for i in range(len(permutation)):
        folium.Marker(
            coordinates[permutation[i]],
            popup=f"{i+1}. {locations[permutation[i]]}",
            icon=folium.Icon(color='red', icon='info-sign')
        ).add_to(m)

    # Plot the route using OSRM
    route_coordinates = []
    for i in range(len(permutation)):
        start = coordinates[permutation[i]]
        end = coordinates[permutation[(i + 1) % len(permutation)]]
        route = get_osrm_route(start, end)
        route_coordinates.extend(route)

    # Convert the route coordinates to folium PolyLine and add to map
    folium.PolyLine([(lat, lon) for lon, lat in route_coordinates], color="blue", weight=2, opacity=0.8).add_to(m)

    m.save("mumbai_optimized_route_new.html")
    print("Map saved as 'mumbai_optimized_route_new.html'")
